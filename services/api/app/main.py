...
def create_app(
    ...
    reimbursement = orchestrator or _default_reimbursement_orchestrator()
    hiker = hiker_orchestrator or HikerOrchestrator()
    associate = associate_orchestrator or _default_associate_orchestrator()
    ...
    if chat_orchestrator is None or create_orchestrator is None:
        run_store = _default_run_store()
        if run_store is not None:
            run_store.mark_stale_interrupted("chat")
            run_store.mark_stale_interrupted("create")
    if run_store is None:
        run_store = _default_run_store()
    create = create_orchestrator or CreateOrchestrator(run_store=run_store)
    memory = memory_store or _default_memory_store()
    # J2 判断力轮:生产装配独立法官(build_judge)开启判断层;测试直接构造编排器时
    # 不装配 judge → 判断层惰性(零评估、字节等价)。settings 显式共享,判官与主循环
    # 同一模型配置。
    chat_settings = RuntimeSettings.from_env()
    ...
    identity = identity_service or _default_identity_service()
    crew = crew_service or _default_crew_service()
    ...
    app.include_router(chat_routes.build_router(chat))
    app.include_router(workdirs_routes.build_router())
    app.include_router(hiker_routes.build_router(hiker))
    v2_bridge = harness_v2_bridge or HarnessV2Bridge.from_env()
    ...
    app.include_router(create_routes.build_router(create))
    app.include_router(reimbursement_routes.build_router(reimbursement))
    app.include_router(
...
def _default_reimbursement_orchestrator() -> ReimbursementOrchestrator:
...
def _default_associate_orchestrator() -> AssociateReceivablesOrchestrator:
    settings = RuntimeSettings.from_env()
    state_store = (
...
def _default_run_store() -> SQLiteRunStore | None:
...
def _default_execution_store() -> SQLiteExecutionStore:
    settings = RuntimeSettings.from_env()
    if settings.state_db_path:
...
def _default_memory_store() -> BusinessMemoryStore:
    settings = RuntimeSettings.from_env()
...
def _default_crew_service() -> CrewService:
    settings = RuntimeSettings.from_env()
