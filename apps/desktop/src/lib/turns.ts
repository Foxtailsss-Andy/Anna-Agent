...
const EVENT_LABELS: Record<string, string> = {
  ...
  // C2:真实审计名的呈现映射(事件名→中文标签,ADR-002 允许;原名仍在 wire)
  'chat.run.created': '接下任务',
  'chat.run.failed': '运行失败',
  'run.suspended': '回合预算用尽 · 已挂起', // L4b:顶到 max_turns 的诚实暂停(回看时间线可读)
  'run.queued': '排队中 · 等待并发额度', // L5:工作空间闸限流的诚实排队
  // 判断力轮(J1/J2/J3):判断层与 steering 的真实审计名 —— 无映射会把裸事件名漏给用户
  'plan.gate.fired': '计划未完 · 已要求继续', // J1 守门真的拦下了一次收尾
  'plan.gate.exhausted': '计划仍未完 · 已诚实放行', // J1 到上限后不死循环,如实放行
  'run.evaluation.started': '判断 · 正在复核是否办妥', // J2
  'run.evaluation.verdict': '判断 · 复核结论',
  'run.evaluation.flagged': '判断 · 未完全达成（已诚实标注）',
  'run.evaluation.skipped': '判断 · 本次未评估',
  'run.interjected': '收到补充指示', // J3:插话回执(真凭证进时间线,不靠前端自说自话)
  'chat.response.generated': '生成回答',
  'chat.artifact.emitted': '产出产物',
  'model.call.started': '模型调用',
  'model.call.completed': '模型调用完成',
  'mcp.tool.called': '外部工具调用',
  // B1:Create 流式管线真实审计名(三 kind 同表)
  'create.skill.run.created': '接下任务',
  'create.prompt.run.created': '接下任务',
  'create.python_tool.run.created': '接下任务',
  'create.skill.generated': '草稿写入',
  'create.prompt.generated': '草稿写入',
  'create.python_tool.generated': '草稿写入',
  'create.skill.validated': '读回校验',
  'create.prompt.validated': '读回校验',
  'create.python_tool.fixture_ran': '沙箱评测',
  'create.skill.saved': '注册入库',
  'create.prompt.saved': '注册入库',
  'create.failed': '运行失败',
};

/** 启动语义的系统步标签(C2:类型前缀「启动」白名单;其余系统步前缀「系统」) */
export const LAUNCH_STEP_LABELS = new Set(['接下任务', '装载 Skill']);

/* ---------------- J2×J3:被还原的内部段失败 ---------------- */

/** 判断层内部段(续办/插话投递)空产出、原答案已被还原时,那行的诚实标签。 */
export const RESTORED_CONTINUATION_LABEL = '补办一轮未产出 · 已保留原答案';

/** 还原守卫之后紧跟的两种 skip 理由(见 chat orchestrator `_continuation_skip_reason`) */
const RESTORED_SKIP_REASONS = new Set(['continuation_incomplete', 'continuation_exhausted']);

/**
 * 找出「其实没让这次 run 失败」的那些 `chat.run.failed` 事件下标。
 *
 * 判断层在宣布办妥前可能再跑一段(续办 / 插话投递)。那一段的模型若返回空文本,
 * capability 会照实写下 `chat.run.failed {error_code: chat_response_empty}`;
 * 随后 orchestrator 还原已交付的终态,并补一条
 * `run.evaluation.skipped {reason: continuation_*}`。事件全是真的,可时间线上
 * 只剩孤零零一行「运行失败」,而卡片写着「办妥」—— 用户没法同时相信这两样。
 *
 * 所以**只在成对出现时**改写标签:一条 `chat_response_empty` 的失败,后面跟着一条
 * 续办跳过。单独出现的 `chat.run.failed` 一个字不改 —— 那才是真的失败
 * (且首段就失败的 run 根本不会进判断层,见 orchestrator `_drive` 的
 * `final_run.status == "ready"` 前置)。顺序也是判据:先失败、后还原。
 */
export function restoredContinuationFailures(frames: Frame[]): Set<number> {
  const pending: number[] = [];
  const restored = new Set<number>();
  frames.forEach((f, i) => {
    if (f.type !== 'event') return;
    if (f.name === 'chat.run.failed' && f.payload?.error_code === 'chat_response_empty') {
      pending.push(i);
      return;
    }
    if (
      f.name === 'run.evaluation.skipped' &&
      RESTORED_SKIP_REASONS.has(String(f.payload?.reason ?? ''))
    ) {
      const idx = pending.pop(); // 最近一条还没配上对的空产出失败
      if (idx !== undefined) restored.add(idx);
    }
  });
  return restored;
}
...
export function reduceTurns(frames: Frame[], labels: ToolLabels = DEFAULT_TOOL_LABELS): RunTree {
  ...
  // 标签判定需要跨帧上下文的唯一一处:被还原的内部段失败(见函数文档)。
  const restoredFailures = restoredContinuationFailures(frames);
    ...
    switch (f.type) {
      case 'event': {
        ...
        t.steps.push({
          id: `s${seq++}`,
          kind: 'system',
          label: restoredFailures.has(fi)
            ? RESTORED_CONTINUATION_LABEL
            : (EVENT_LABELS[f.name] ?? f.name),
          status: 'ok',
        });
        break;
      }
