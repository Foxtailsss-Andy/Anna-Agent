...
export interface ToolDrilldown {
  ...
  contract?: string;
  /**
   * C2 增量(校对基准 P-03 琥珀注):载荷缺口的真实说明,
   * 如「入参/回执全文 · 待审计载荷扩展(B2)」。可选,向后兼容。
   */
  pendingNote?: string;
}
...
export interface EventFrame {
  ...
  turn?: number;
  /**
   * 审计载荷原样透传(后端 `event.payload`)。**只用于标签判定**,不进 L3
   * —— 系统步「无 L3、不可掀」的硬规则不动。
   * 需要它的原因:有些事件名单看不出真相,`chat.run.failed{error_code:
   * chat_response_empty}` 就是一例(它可能是真失败,也可能是判断层内部段的空产出
   * 且已被还原)。差别只在载荷里,不给载荷就只能在两种情况里猜一种。
   */
